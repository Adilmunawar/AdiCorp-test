    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-3">
          {records?.map((rec: any) => (
            <div key={rec.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors border border-border/50">
              ...
              <div className="flex items-center gap-3">
                <Badge className={statusColors[rec.status] || ""} variant="outline">{rec.status}</Badge>
                {isAdmin && rec.status === "pending" && (
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" className="text-green-600 hover:bg-green-500/10" onClick={() => handleAction(rec.id, "approved")}><Check className="h-4 w-4" /></Button>
                    <Button size="sm" variant="ghost" className="text-red-600 hover:bg-red-500/10" onClick={() => handleAction(rec.id, "rejected")}><X className="h-4 w-4" /></Button>
                  </div>
                )}
              </div>
            </div>
