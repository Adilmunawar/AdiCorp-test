...
export default function EmployeeForm({ employee, onSuccess, onCancel, isOpen, onClose, employeeId }: EmployeeFormProps) {
  ...
  const form = useForm<EmployeeFormData>({
    ...
    defaultValues: { name: "", rank: "", wage_rate: 0, status: "active" },
  });

  useEffect(() => {
    if (currentEmployee) {
      form.reset({ name: currentEmployee.name || "", rank: currentEmployee.rank || "", wage_rate: currentEmployee.wage_rate || 0, status: currentEmployee.status === "inactive" ? "inactive" : "active" });
    } else {
      form.reset({ name: "", rank: "", wage_rate: 0, status: "active" });
    }
  ...
  const handleSubmit = async (data: EmployeeFormData) => {
    ...
    try {
      if (currentEmployee) {
        const { error } = await supabase.from("employees").update({ name: data.name, rank: data.rank, wage_rate: data.wage_rate, status: data.status }).eq("id", currentEmployee.id);
        if (error) throw error;
        await logEmployeeActivity('update', data.name, { employee_id: currentEmployee.id, previous_name: currentEmployee.name, previous_rank: currentEmployee.rank, previous_wage_rate: currentEmployee.wage_rate, new_rank: data.rank, new_wage_rate: data.wage_rate, status_change: currentEmployee.status !== data.status ? `${currentEmployee.status} → ${data.status}` : 'No change' });
        toast.success(`Employee ${data.name} updated successfully`);
      } else {
        const { data: newEmployee, error } = await supabase.from("employees").insert({ company_id: userProfile.company_id, name: data.name, rank: data.rank, wage_rate: data.wage_rate, status: data.status }).select().single();
        if (error) throw error;
        await logEmployeeActivity('create', data.name, { employee_id: newEmployee.id, rank: data.rank, wage_rate: data.wage_rate, status: data.status, creation_time: new Date().toISOString() });
